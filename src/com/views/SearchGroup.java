...
public class SearchGroup extends VerticalLayout implements ValueChangeListener,
		View {
	...
	public void valueChange(ValueChangeEvent event) {
		...
		try {
			...
			System.out.println(owner.getGroupNumber()+"------>");
			ArrayList<GroupPolicy> policies = GRPDBUtility.getPolicyAllDetails(searchTerm.getValue().toString());
			GroupForm cus = new GroupForm(owner,sheet);
			sheet.addTab(cus, "Customer BIODATA");
			for (GroupPolicy policy : policies){
			System.out.println(policy.getGroupPolicyNumber()+"------>");
